...
// Версия: 12.1 (ИСПРАВЛЕННАЯ логика расчета по периодичности)
// ============================================
...
function calculateCostPer4Weeks(pricePerReplacement, quantity, frequency) {
    ...
    // Цена НЕ зависит от частоты замены - зависит только от размера и региона
...
function getFallbackPrice(size, frequency) {
    // Базовые цены за ОДНУ замену ОДНОГО ковра
    // ВНИМАНИЕ: Цена НЕ зависит от частоты замены!
    // Частота влияет только на количество замен в формуле расчета
    const basePrices = {
        "85*60": 500,
        "85*150": 800,
        "115*200": 1200,
    ...
    // Цена за одну замену всегда одинакова для всех частот
    // Не важно: "1 раз в две недели", "1 раз в неделю" или "7 раз в неделю"
    // Разница будет только в количестве замен в формуле calculateCostPer4Weeks()
