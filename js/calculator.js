...
function sendToTelegram() {
    ...
    try {
        ...
        // Проверяем длину URL (ограничение для браузеров ~2000 символов)
        const telegramUrl = `https://t.me/+79770005127?text=${encodedMessage}`;
        
        if (telegramUrl.length > 2000) {
            // Если URL слишком длинный, создаем еще более короткое сообщение
            console.warn('URL слишком длинный, сокращаем сообщение');
            const shortMessage = createVeryShortTelegramMessage();
        ...
        setTimeout(() => {
            if (isMobile) {
                ...
                alert('Telegram открыт в новом окне! Нажмите "Отправить" чтобы отправить расчет.\n\nМы свяжемся с вами в течение 15 минут.');
            }
        }, 1000);
...
function sendToEmail() {
    ...
    try {
        ...
        const body = encodeURIComponent(createShortEmailMessage());
        ...
        if (emailUrl.length > 2000) {
            ...
            const shortBody = encodeURIComponent(createVeryShortEmailMessage());
            const shortUrl = `mailto:matservice@yandex.ru?subject=${subject}&body=${shortBody}`;
            
            if (shortUrl.length > 2000) {
                alert('Сообщение слишком длинное. Пожалуйста, отправьте email на matservice@yandex.ru');
                return;
            }
